...
#### 关于线程 ####
* 一个计时器线程`ThreadPool::startTimerThread`, 负责超时检测/安全检查 
...
* `sendMessage(long from, long to, byte mtype, String msg, String attrs, int timeout, FPCallback.ICallback callback)`: 发送消息
    * `from`: **(long)** 发送方 id
    * `to`: **(long)** 接收方uid
    * `mtype`: **(byte)** 消息类型
    * `msg`: **(String)** 消息内容
...
* `sendMessages(long from, List<Long> tos, byte mtype, String msg, String attrs, int timeout, FPCallback.ICallback callback)`: 发送多人消息
    * `from`: **(long)** 发送方 id
...
* `sendGroupMessage(long from, long gid, byte mtype, String msg, String attrs, int timeout, FPCallback.ICallback callback)`: 发送group消息
    * `from`: **(long)** 发送方 id
    * `gid`: **(long)** group id
    * `mtype`: **(byte)** 消息类型
    * `msg`: **(String)** 消息内容
...
* `sendRoomMessage(long from, long rid, byte mtype, String msg, String attrs, int timeout, FPCallback.ICallback callback)`: 发送room消息
    * `from`: **(long)** 发送方 id
    * `rid`: **(long)** room id
    * `mtype`: **(byte)** 消息类型
    * `msg`: **(String)** 消息内容
...
* `broadcastMessage(long from, byte mtype, String msg, String attrs, int timeout, FPCallback.ICallback callback)`: 广播消息(andmin id)
    * `from`: **(long)** admin id
    * `mtype`: **(byte)** 消息类型
    * `msg`: **(String)** 消息内容
    * `attrs`: **(String)** 消息附加信息, 可传`""`
...
* `addFriends(long uid, List<Long> friends, int timeout, FPCallback.ICallback callback)`: 添加好友
    * `uid`: **(long)** 用户 id
...
* `deleteFriends(long uid, List<Long> friends, int timeout, FPCallback.ICallback callback)`: 删除好友
    * `uid`: **(long)** 用户 id
    * `friends`: **(List<Long>)** 多个好友 id
    * `timeout`: **(int)** 超时时间(ms)
    * `callback`: **(FPCallback.ICallback)** 回调方法
...
* `isFriend(long uid, long fuid, int timeout, FPCallback.ICallback callback)`: 是否好友
    * `uid`: **(long)** 用户 id
    * `fuid`: **(long)** 好友 id
    * `timeout`: **(int)** 超时时间(ms)
    * `callback`: **(FPCallback.ICallback)** 回调方法
...
* `isFriends(long uid, List<Long> fuids, int timeout, FPCallback.ICallback callback)`: 是否好友
    * `uid`: **(long)** 用户 id
...
* `getToken(long uid, int timeout, FPCallback.ICallback callback)`: 获取token
    * `uid`: **(long)** 用户 id
    * `timeout`: **(int)** 超时时间(ms)
    * `callback`: **(FPCallback.ICallback)** 回调方法
        * `exception`: **(Exception)** 
...
* `setGeo(long uid, double lat, double lng, int timeout, FPCallback.ICallback callback)`: 设置位置
    * `uid`: **(long)** 用户 id
    * `lat`: **(double)** 纬度
    * `lng`: **(double)** 经度
    * `timeout`: **(int)** 超时时间(ms)
...
* `getGeo(long uid, int timeout, FPCallback.ICallback callback)`: 获取位置
    * `uid`: **(long)** 用户 id
    * `timeout`: **(int)** 超时时间(ms)
    * `callback`: **(FPCallback.ICallback)** 回调方法
        * `exception`: **(Exception)** 
...
* `sendFile(long from, long to, byte mtype, String filePath, int timeout, FPCallback.ICallback callback)`: 发送文件
    * `from`: **(long)** 发送方 id
    * `to`: **(long)** 接收方uid
    * `mtype`: **(byte)** 消息类型
...
* `sendFile(long from, long to, byte mtype, byte[] fileBytes, int timeout, FPCallback.ICallback callback)`: 发送文件
    * `from`: **(long)** 发送方 id
    * `to`: **(long)** 接收方uid
    * `mtype`: **(byte)** 消息类型
    * `fileBytes`: **(byte[])** 文件内容
